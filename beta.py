...
def learn(wall, lava, train=True, tpu_train=False, graph=True, display=True):
    SIZE = 10
    train = train
    ...
    agent = AgentModel(SIZE, REPLAY_MEMORY_SIZE, MIN_REPLAY_SIZE, MINIBATCH_SIZE, GAMMA, UPDATE_TARGET_EVERY,
                       LOAD_MODEL)
    rewards_list = []
    env = MyEnv(SIZE, ENEMY_P, MOVE_P, FOOD_R, LAVA_P, wall_type=wall, lava_type=lava)
...
walls = ['special', 'bottom', 'top']
lavas = ['one', 'two', 'three']
for wall in walls:
    for lava in lavas:
        learn(wall=wall, lava=lava, train=True, tpu_train=False, graph=False, display=False)
